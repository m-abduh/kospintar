...
waWebhookRouter.post("/:instanceName", async (req, res) => {
  try {
    ...
    if (event === "messages.upsert") {
      ...
      if (!tenant) {
        await fetch(`${config.evolution.url}/message/sendText/${instance}`, {
          method: "POST",
          headers: { "Content-Type": "application/json", apikey: config.evolution.api_key },
          body: JSON.stringify({ number: from, text: "Halo, untuk informasi lebih lanjut silakan hubungi pengelola." }),
        });
        res.status(200).json({ status: "ok" });
        return;
      }
